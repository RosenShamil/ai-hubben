  ...
  return (
    <>
          ...
          <SectionCard>
            ...
            <div className="mt-8 h-[300px] md:h-[380px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={currentDailyInteractions.reduce<Array<{ date: string; interactions: number }>>((acc, d) => {
                    const prev = acc.length > 0 ? acc[acc.length - 1].interactions : 0;
                    acc.push({ date: d.date, interactions: prev + d.interactions });
                    return acc;
                  }, [])}
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <FadeIn>
            <SectionCard className="h-full">
              ...
              {/* Show dominant #1 as a callout, chart the rest */}
              {currentTopAssistants.length > 0 && (() => {
                const sorted = [...currentTopAssistants].sort((a, b) => b.messages - a.messages);
                const top = sorted[0];
                const rest = sorted.slice(1);
                return (
                  <>
                    <div className="mt-6 flex items-baseline justify-between rounded-md border border-border bg-background px-4 py-3">
                      <span className="text-[0.8125rem] text-muted-foreground" style={{ fontFamily: "var(--font-geist-mono), monospace" }}>
                        #1 {top.name}
                      </span>
                      <span className="text-[1.5rem] tracking-[-0.04em]" style={{ fontFamily: "var(--font-bodoni), serif", fontWeight: 400 }}>
                        {top.messages.toLocaleString("sv-SE")}
                      </span>
                    </div>
                    <div className="mt-4 h-[320px]">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <XAxis type="number" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={{ stroke: "var(--border)" }} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                          <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} width={180} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                          <Tooltip content={<ChartTooltip />} />
                          <Bar dataKey="messages" name="Meddelanden" radius={[0, 4, 4, 0]}>
                            {[...rest].reverse().map((_, index) => (
          ...
          <FadeIn delay={0.15}>
            <SectionCard>
              ...
              {currentFileUploads.length > 0 && (() => {
                const sorted = [...currentFileUploads].sort((a, b) => b.count - a.count);
                const top = sorted[0];
                const rest = sorted.slice(1);
                return (
                  <>
                    <div className="mt-6 flex items-baseline justify-between rounded-md border border-border bg-background px-4 py-3">
                      ...
                      <span className="text-[1.5rem] tracking-[-0.04em]" style={{ fontFamily: "var(--font-bodoni), serif", fontWeight: 400 }}>
                        {top.count.toLocaleString("sv-SE")}
                      </span>
                    </div>
                    <div className="mt-4 h-[320px]">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <XAxis dataKey="type" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={{ stroke: "var(--border)" }} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                          <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                          <Tooltip content={<ChartTooltip />} />
        ...
        <FadeIn>
          <SectionCard>
            ...
            {yearComparison.length > 0 && (() => {
              const sorted = [...yearComparison].sort((a, b) => Math.max(b["2025"], b["2026"]) - Math.max(a["2025"], a["2026"]));
              const top = sorted[0];
              const rest = sorted.slice(1);
              return (
                <>
                  <div className="mt-6 flex items-center justify-between rounded-md border border-border bg-background px-4 py-3">
                    ...
                    <div className="flex items-baseline gap-4">
                      <span className="text-[0.75rem] text-muted-foreground" style={{ fontFamily: "var(--font-geist-mono), monospace" }}>
                        <span className="mr-1.5 inline-block h-2 w-2 rounded-full" style={{ backgroundColor: CHART_COLORS[4] }} />
                        2025: {top["2025"].toLocaleString("sv-SE")}
                      </span>
                      <span className="text-[0.75rem] text-muted-foreground" style={{ fontFamily: "var(--font-geist-mono), monospace" }}>
                        <span className="mr-1.5 inline-block h-2 w-2 rounded-full" style={{ backgroundColor: CHART_COLORS[0] }} />
                        2026: {top["2026"].toLocaleString("sv-SE")}
                      </span>
                    </div>
                  </div>
                  <div className="mt-4 h-[320px] md:h-[340px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={rest}
                      ...
                      >
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                        <XAxis dataKey="category" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={{ stroke: "var(--border)" }} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                        <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} tickLine={false} axisLine={false} style={{ fontFamily: "var(--font-geist-mono), monospace" }} />
                        <Tooltip content={<ChartTooltip />} />
                        <Legend
                          verticalAlign="top"
      ...
      <section className="mx-auto max-w-[68.75rem] px-6 py-12 md:py-16">
        ...
        {/* Förvaltning/yrkesgrupp-diagram dolt — data finns kvar i trainingStats */}
      </section>
